...
<details>
<summary>1.2.15 Командные файлы в Bourne Shell</summary>

- Командный файл или «скрипт» - файл, начинающийся с #!путь_к_интерпретатору и имеющий право на исполнение («execute» бит, chmod +x), пример !#/bin/sh (или !#/usr/bin/perl)
- В Unix-е исполняемые файлы бывают 2-х видов: бинарники и скрипты
- Для запуска скрипта перед файлом-скриптом пишется «./» (текущий каталог), пример ./scrpipt.sh
- Почему нельзя просто указать название файла-скрипта (без «./», script.sh)?  
В отношении имён файлов, содержащих программы, которые нужно запустить, действует соглашение, отличающееся от соглашения обычных файловых имён: имена таких файлов - это имена команд (ls, cd, cat, mv…), интерпретатор такие команды выполняет сам, если это встроенная команда, иначе он ищет файл в системных каталогах Поэтому мы не может использовать только имя файла как имя команда (чтобы интерпретатор не искал в системных каталогах файл, так как он находится в текущем каталоге), а добавляем перед именем скрипта относительный путь, чтобы интерпретатор не искал ничего, а сразу выполнял // В имени скрипта должен присутствовать хотя бы один слэш, чтобы интерпретатор не искал (пример: путь/файл, ./файл) 
- Объявление переменной (вокруг знака присвоения не должен быть пробел):  `T=10 FILE=/path/file …`   Обращение к переменной: $ + ИМЯ_ПЕРЕМЕННОЙ  `echo $T $FILE // => получим «10 /path/file»`   Арифметика: `$((ВЫРАЖЕНИЕ)) A=10 T=$((10 + $A)) // T=20`   Параметры командной строки скрипта: $0 - имя скрипта, $# - количество аргументов ` #!/bin/sh echo «script name: » $0 «with » $# «parameters» echo «» «$3 $2 $1»  ` `>> ./script.sh abra habra kabra`  Получим:  `script name: ./script.sh with 3 parameters // то есть - abra, habra, kabra kabra habra abra  ` В последней строке скрипта script.sh после echo сразу идет «» (пустой) параметр, чтобы если вдруг после echo сразу поставят параметр начинающийся с минуса не произошло что-то непредсказуемое 
- Знак $ не теряет свой смысл внутри двойных скобок («$2» -> второй параметр скрипта) и теряет внутри апострофа (‘$2’ -> $2)
- Любые команды завершаются успешно или неуспешно, поэтому любая программа должна сообщать ядру об успешности завершения в виде «кода завершения»
- Код завершения - Целое число в диапазоне от 0 до 255: 0 - успешно, от 1 до 255 - неуспешно 
- [ -f «file.txt» ] или test -f «file.txt»  - существует ли файл с именем file.txt  if [ -f «file.txt» ]; then     cat «file.txt» else     echo «file.txt not found» fi  if mkdir folder; then     echo «Yes» else      echo «No» fi  A=1 while [$A -le 10]; do     echo $A     A=$((A+1)) done  -le (less than)  cmd1 && cmd2 (cmd2 сработает если cmd1 сработает) cmd1 || cmd2 (сработает cmd2 в случае невыполнения cmd1) cmd1 && cmd2 | cmd3 (сначала (конвейер сработает) cmd2 | cmd3 потом cmd1 && результат конвейера)
</details>
